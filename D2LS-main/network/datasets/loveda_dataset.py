...
class LoveDATrainDataset(Dataset):
    def __init__(self, data_root='data/LoveDA/Train/train_val/Urban', img_dir='images_png', mosaic_ratio=0.25,
    ...
    def __getitem__(self, index):
        ...
        # === 新增：尝试加载 SAM masks（仅训练集可能存在）===
        img_id, img_type = self.img_ids[index]
        base_name = img_id
        sam_dir = osp.join(self.data_root, '..', 'sam_masks')  # data/LoveDA/sam_masks
        sgo_path = osp.join(sam_dir, base_name + '_sgo.png')
        sgb_path = osp.join(sam_dir, base_name + '_sgb.png')

        sgo = None
        sgb = None
        # 只在训练集目录下尝试加载（val 不会有文件）
        if osp.exists(sgo_path):
            sgo = np.array(Image.open(sgo_path)).astype(np.int64)
        if osp.exists(sgb_path):
            sgb = np.array(Image.open(sgb_path)).astype(np.float32)
            if sgb.max() > 1.0:
                sgb /= 255.0
        ...
        results = {
            ...
            'img_id': img_id,
            'img_type': img_type,
            # === 关键：无论训练/验证，都返回这两个 key ===
            'sgo': torch.from_numpy(sgo).long() if sgo is not None else None,
            'sgb': torch.from_numpy(sgb).float() if sgb is not None else None
        }
        return results
